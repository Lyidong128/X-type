...
def _compute_row(
    ...
    wilson = compute_wilson_det_result(
        model,
    ...
    z2 = int(wilson.z2)
    z2_reliable = int(min(gap.direct_gap, gap.indirect_gap) >= 1e-4)
    ...
    comment = ""
    if min(gap.direct_gap, gap.indirect_gap) < 1e-4:
        comment = "gap below reliability threshold"
    return {
        "v": float(v),
        "t": float(t),
        "w": float(w),
        ...
        "comment": comment,
        "wilson_winding": float(wilson.winding),
